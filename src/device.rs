...
impl Device {
    ...
    pub fn open() -> Result<Self, Error> {
        let device = Self {
            transport: Transport::open(RTL_VID, RTL_PID)?,
        };
        device.init()?;
        //device.detect_tuner()?;
        device.init_tuner()?;
    ...
    fn init(&self) -> Result<(), Error> {
        ...
        // FIXME The datasheet gives 0x35D82E as the example value for IF 4.57 MHz
        // I suspect we should match that, since the working value begins 0x3C
        // TODO Write a program that shifts this number up and down over time so I can view it's
        // effect.
        // TODO I think we can also do it in one write. A single &[0x3c, 0x99] might work.
    ...
    fn setup_usb_endpoint(&self) -> Result<(), Error> {
        self.transport.write_reg(Block::Usb, 0x2000, &[0x09])?;
        Ok(())
    }

    fn init_demod(&self) -> Result<(), Error> {
    ...
    fn init_tuner(&self) -> Result<(), Error> {
        ...
        // TODO reg 0x1e also does power detector timing control. check value for this.
        self.transport
    ...
    pub fn configure(self, config: Config) -> Result<ConfiguredDevice, Error> {
...
impl ConfiguredDevice {
    ...
    ///
    /// Will return `Err` if there are any USB errors during the process.
    pub fn sample(&self, duration: Duration) -> Result<Vec<u8>, Error> {
        let mut reader = self.transport.bulk_reader()?;

        let mut iq = Vec::new();

        let seconds = duration.as_secs();
        let samples = u64::from(self.config.sample_rate) * seconds * 2;
